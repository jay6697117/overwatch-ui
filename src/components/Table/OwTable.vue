        ...
        <thead>
          <tr>
            ...
            <th v-if="selectable" class="ow-table-center" :style="{ width: '50px' }">
              <input ref="allCheckbox" type="checkbox" :checked="areAllItemsSelected" @change="onSelectAllItems" />
            </th>
            <th class="ow-table-center" :style="{ width: '50px' }" v-if="isShowIndex">#</th>
            <th
            ...
            >
              <div>
                ...
                <span v-if="column.field in sortRules" class="ow-table-sort-indicator">
                  <ow-icon :color="getAscColor(column.field)" size="10px" name="up-solid"></ow-icon>
                  <ow-icon :color="getDescColor(column.field)" size="10px" name="down-solid"></ow-icon>
                </span>
              </div>
            </th>
        ...
        <tbody>
          <template v-for="(item, index) in source">
            <tr :key="item.id">
              <td
                v-if="expandField"
                class="ow-table-expander ow-table-center"
              ...
              <td v-if="selectable" :style="{ width: '50px' }" class="ow-table-center">
                <input type="checkbox" :checked="isItemSelected(item)" @change="onSelectItem(index, item, $event)" />
              </td>
...
export default {
  ...
  watch: {
    selected() {
      this.$refs.allCheckbox.indeterminate = !(
        this.selected.length === this.source.length || this.selected.length === 0
      );
    }
  },
  components: {
