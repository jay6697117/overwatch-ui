<template>
  <button :class="classes" @click="$emit('click')">
    <ow-icon class="ow-button-icon" color="#fff" :is-loading="isLoading" :name="iconName"></ow-icon>
...
export default {
  ...
  computed: {
    classes() {
      return ['ow-button', { [`ow-button-icon-${this.iconPosition}`]: true }, `ow-button-${this.type}`];
    }
  },
  components: {
    OwIcon
