  ...
  methods: {
    listenToRemoteResources() {
      ...
      Array.from(tags).map(tag => {
        if (tag.hasAttribute('data-ow-listened')) {
          return;
        }
    ...
    calculateContentYMax() {
      ...
      let maxHeight =
        this.childHeight - this.parentHeight + (borderTopWidth + borderBottomWidth + paddingTop + paddingBottom);
      return maxHeight;
    },
    updateScrollBar() {
...
.ow-scroll {
  ...
  &-bar {
    ...
    &-inner {
      height: 100%;
      border-radius: 4px;
      background: $--scrollbar-inner-background-color;
      &:hover {
