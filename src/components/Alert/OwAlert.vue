  ...
  <div class="ow-alert">
    <div class="ow-alert-content">
      <span class="ow-alert-content-icon">
        <ow-icon :name="iconName" :is-loading="isLoading" color="#ffffff"></ow-icon>
      </span>
      <div :class="textClasses">
        <p>
          <strong class="ow-alert-content-title">{{ title }}</strong>
        </p>
        <p class="ow-alert-content-message">{{ message }}</p>
      </div>
    </div>
    <div v-if="autoCloseDelay < 0" class="ow-alert-cancel">
      <ow-button @click="onClickClose" class="ow-alert-cancel-button" :type="buttonType">
        {{ closeButton.text }}
      </ow-button>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    textClasses() {
      return ['ow-alert-content-text', `ow-alert-content-text-${this.type}`];
    }
  },
  mounted() {
    this.execAutoClose();
...
.ow-alert {
  ...
  animation: owAlert-slide-down 0.3s;
  ...
  &-content {
    ...
    &-text {
      ...
      &-info,
      &-loading {
        border-left: 2px solid $--alert-info-color;
      }
      &-danger {
        border-left: 2px solid $--alert-danger-color;
      }
      &-success {
        border-left: 2px solid $--alert-success-color;
      }
      &-warning {
        border-left: 2px solid $--alert-warning-color;
      }
    }
    &-title {
