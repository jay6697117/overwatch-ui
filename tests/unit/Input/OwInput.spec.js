  ...
  describe('props', () => {
    ...
    it('renders props.value when passed', () => {
      ...
      testProperty(values, value => {
        ...
        expect(OwInputWrapper.find('input').attributes('type')).to.equal(value);
      });
    });
    it('renders props.disabled when passed', () => {
      ...
      expect(OwInputWrapper.find('input').element.getAttribute('disabled')).to.equal('disabled');
    });
    it('renders props.readonly when passed', () => {
      const { readonly } = testData;
      OwInputWrapper.setProps({ readonly });

      expect(OwInputWrapper.find('input').element.getAttribute('readonly')).to.equal('readonly');
    });
    it('renders props.hint when passed', () => {
      ...
      hints.forEach(hint => {
        ...
        expect(OwInputWrapper.find('small').text()).to.equal(hint);
      });
    });
