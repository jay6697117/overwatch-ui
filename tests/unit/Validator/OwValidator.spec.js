...
describe('Validator', () => {
  ...
  it('test wrong error content', () => {
    ...
    const rules = [{ field: 'email', pattern: /^.+@.+$/ }];
    const validator = new OwValidator();
    const errors = validator.validate(data, rules);
  ...
  it('test email keyword for invalid email', () => {
    ...
    const rules = [{ field: 'email', pattern: 'email' }];
    const validator = new OwValidator();
    const errors = validator.validate(data, rules);
    expect(errors.email.pattern).to.equal('Invalid value');
  });
  ...
  it('test required and pattern keywords', () => {
    ...
    const rules = [{ field: 'email', pattern: 'email', required: true }];
    const validator = new OwValidator();
    const errors = validator.validate(data, rules);
  ...
  it('test pattern and minLength', () => {
    ...
    const rules = [{ field: 'email', pattern: 'email', minLength: 6 }];
    const validator = new OwValidator();
    const errors = validator.validate(data, rules);
    // errors = {email: {pattern: 'Invalid', required: ''}}
  ...
  it('test maxLength', () => {
    ...
    const rules = [{ field: 'email', pattern: 'email', maxLength: 10 }];
    const validator = new OwValidator();
    const errors = validator.validate(data, rules);
